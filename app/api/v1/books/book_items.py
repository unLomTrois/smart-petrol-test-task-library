...
@router.post("/{book_id}/add/{book_count}",
             dependencies=[Depends(check_role(["librarian"]))])
async def add_book_item(book_item: book_schemas.BookItemCreate,
                        db: Session = Depends(get_db)):
    return crud.add_book_items(db=db,
                               book_id=book_item.parent_book_id,
                               book_count=book_item.count)
