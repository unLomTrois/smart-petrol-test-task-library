...
def get_issue_by_book_item(db: Session, book_item_id: int):
    return db.query(models.BookIssue).filter(
        models.BookIssue.book_item_id == book_item_id).first()


def get_all_issued_books(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.BookItem).filter(
        models.BookItem.is_given == True).offset(skip).limit(limit).all()
...
def issue_a_book(db: Session, form: IssueBookForm):
    ...
    book_item = get_book_item(db=db, book_item_id=form.book_item_id)

    if is_booked(
            db=db, book_item_id=form.book_item_id) and not is_booked_for_user(
                db=db, book_item_id=form.book_item_id, user_id=form.user_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=
            "Вы не можете выдать книгу пользователю, если она уже забронирована не для него",
        )
    ...
    new_issue = models.BookIssue(user=user,
                                 book_item=book_item,
                                 end_of_issue=form.end_of_issue)
    # update book item
